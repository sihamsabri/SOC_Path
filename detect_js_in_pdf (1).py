...
class PDFAnalyzer:
    ...
    # To return in case of a benign file!
        ...
        pars = subprocess.check_output([sys.executable, "%s"%self.pdfparser_path, "%s"%self.file_path,"--search","JavaScript"]).decode("utf-8")
        if pars == "" :
            pars = subprocess.check_output([sys.executable, "%s"%self.pdfparser_path, "%s"%self.file_path,"--searchstream","JavaScript"]).decode("utf-8")
...
#The below section of the handle recusively the search of sections to extract for further analysis
                    ...
                    while(refer != ""):
                        ...
                        if refer_list == ['']:
                            object_var = result_dic[0]["obj"].split(" ")
                            num_ob = object_var[0]
                            print(num_ob)
                            subprocess.check_output([sys.executable, "%s"%self.pdfparser_path, "%s"%self.file_path,"--object", "%s"%num_ob,"--filter","--raw","-d","%s"%filename])
                            #os.system("python3 /opt/remnux-didier/pdf-parser.py {} --object {} --filter --raw -d {}".format(self.file_path,num_ob, filename))
                            #this second command is to handle the case where the FlateDecode option isn't used on the object
                            subprocess.check_output([sys.executable, "%s"%self.pdfparser_path, "%s"%self.file_path,"--object", "%s"%num_ob,"--raw","-d","%s"%filenameprime])
                ...
                if os.path.getsize(file_path) > 0:  # Check if file is not empty
                    with open(file_path, 'r') as file:
                        ...
                        if 'No Filter' not in content:  
                            ...
                            if matches:
                                print("Encoded Shellcode Detected inside the file: %s"%filename)
                                return("Malicious Js code detected")
                            ...
                            # The code below find possible malicious URI in the js code
                                ...
                                if url_match:
                                    ...
                                    if result_analysis != "clean link" :
                                        print(f"Malicious link Detected inside the file: %s"%filename)
                                        return("Malicious Js code detected")
...
if javascript_patterns != {}:
    for pattern, matched in javascript_patterns.items():
        ...
        if matched and (pattern == var):
            print("JavaScript pattern '%s' detected in the PDF file.\n This is a malicious PDF"%pattern)
        elif matched and (pattern != var) :
            print("JavaScript pattern '%s' detected in the PDF file.\n This is a suspicious PDF"%parttern)
else:
    analyzer.detect_javascript_with_pdfparser()
